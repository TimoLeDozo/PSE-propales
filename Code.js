...
function setApiKey(key) {
  if (!key || (!/^sk-/.test(key) && key.length < 20)) {
    throw new Error('Cle API DeepSeek invalide.');
  }
  PropertiesService.getScriptProperties().setProperty(PROP_DEEPSEEK_API_KEY, key.trim());
  return { ok: true };
}

function hasApiKey() {
  var k = PropertiesService.getScriptProperties().getProperty(PROP_DEEPSEEK_API_KEY);
  return { hasKey: !!k };
}
...
function callLLM_(provider, prompt, systemPrompt, temperature, options) {
  ...
  try {
    ...
    return { success: false, error: 'Erreur DeepSeek: ' + String(err.message || err) };
  }
  ...
  try {
    ...
    return { success: false, error: 'Reponse DeepSeek invalide: ' + String(err.message || err) };
  }
...
function calculateUsageCost_(usage, model) {
  var pricing = DEEPSEEK_PRICING[model] || DEEPSEEK_PRICING[DEFAULT_DEEPSEEK_MODEL];
  var prompt = usage && usage.prompt_tokens ? usage.prompt_tokens : 0;
  var completion = usage && usage.completion_tokens ? usage.completion_tokens : 0;
...
const ICAM_LOGO_FALLBACK =
  ...
  'U0PTfFUwrmLs6xdX1xyunIO6z+vkuJbmnEmXzyj1ktHvSdpsY4Z9J07bt0DjENkz1H+0Akuv0x3UeHgmzaFZHfHoo//++6w77XfWa17bxnxha70+t7i2pCBc' +
...
function createTemplateCopy(entrepriseNom) {
  try {
    ...
    var name = ('Proposition_' + (entrepriseNom || 'Client') + '_' + ts).replace(/[^a-zA-Z0-9_-]/g, '_');
    var copy = tpl.makeCopy(name, folder);
    return { success: true, documentId: copy.getId(), url: 'https://docs.google.com/document/d/' + copy.getId() + '/edit' };
  } catch (err) {
    return { success: false, error: String(err.message || err) };
  }
}
...
function applyUpdatesToDoc_(docId, updates, options) {
  ...
  var containers = [doc.getBody(), doc.getHeader(), doc.getFooter()].filter(function (x) { return !!x; });

  var escRegex = function (s) { return String(s).replace(/[.*+?^${}()|[\]\]/g, '\\$&'); };
  ...
  function safeInsert(textEl, at, str) {
    ...
    var len = textEl.getText().length;
    if (len === 0) { textEl.setText(insert); return; }
    if (at < 0) at = 0;
    if (at > len) at = len;
  ...
  Object.keys(updates || {}).forEach(function (field) {
    ...
    var isLogo = (field === 'entrepriseLogo') && /^https?:\/\//i.test(String(val || ''));
    ...
    containers.forEach(function (el) {
      ...
      while ((r = el.findText(patt))) {
        ...
        try { safeSetBg(t, s0, e0, null); } catch (_) { }

        safeDelete(t, s0, e0);
        if (val != null && String(val).length) {
          if (isLogo) {
            try {
              var blob = UrlFetchApp.fetch(val, { muteHttpExceptions: true, followRedirects: true }).getBlob();
              t.getParent().asParagraph().insertInlineImage(0, blob).setWidth(120);
            } catch (err) {
      ...
      (function walk(node) {
        ...
        if (type === DocumentApp.ElementType.PARAGRAPH) {
          ...
          for (var i = 0; i < para.getNumChildren(); i++) {
            ...
            if (ch.getType() === DocumentApp.ElementType.TEXT) texts.push(ch.asText());
          }
          if (texts.length) fn(para, texts);
        } else if (node.getNumChildren) {
          for (var j = 0; j < node.getNumChildren(); j++) walk(node.getChild(j));
        }
      })(root);
    });
  }
  function colorAt(textEl, idx) { return normalizeColorHex(textEl.getBackgroundColor(idx)); }
  function isWs(ch) { return !ch || /\s/.test(ch); }
  function prevChar(texts, tIdx, off) {
    if (off > 0) return texts[tIdx].getText().charAt(off - 1) || '';
    if (tIdx > 0) { var s = texts[tIdx - 1].getText(); return s ? s.charAt(s.length - 1) : ''; }
    return '';
  }
  var PUNCT_RE = /^[\s\.,;:!\?\(\)"'\\\-]*$/;
  ...
  function deleteOneSpaceBefore(texts, tIdx, off) {
    if (off > 0) {
      ...
      var p = texts[tIdx - 1];
      var s = p.getText();
      if (s && /\s/.test(s.charAt(s.length - 1))) safeDelete(p, s.length - 1, s.length - 1);
    }
  }

  forEachParagraphTexts(function (para, texts) {
    var lastField = null;
    var lastT = -1, lastEnd = -1;
    ...
    for (var tIdx = 0; tIdx < texts.length; tIdx++) {
      ...
      while (true) {
        ...
        if (!isWs(before)) { safeInsert(textEl, i, ' '); i++; j++; }

        if (UX_SUPPRESS_COMPANY_AFTER_CONTACT && field === 'entrepriseNom' && lastField === 'clientNom') {
          var onlyWs = isOnlyWhitespaceBetween(texts, lastT, lastEnd + 1, tIdx, i);
          if (onlyWs) {
        ...
        if (val != null && String(val).length) {
          ...
          if (sVal.length > 0) try { safeSetBg(textEl, i, i + sVal.length - 1, null); } catch (_) { }
          ...
          if (afterIdx < thisTxt.length) {
            ...
            if (nextEl.getText().length > 0) {
              ...
              if (nextColor2 && MAPS.colorToField[nextColor2]) safeInsert(nextEl, 0, ' ');
            }
          }

          lastField = field;
          lastT = tIdx; lastEnd = i + sVal.length - 1;
          i = lastEnd + 1;
        } else {
          safeDelete(textEl, i, j - 1);
          lastField = field;
      ...
      function walkElm(el) {
        if (el.getType && el.getType() === DocumentApp.ElementType.TEXT) {
          ...
          for (var k = 0; k < len; k++) {
            ...
            if (known.indexOf(c) !== -1) { try { safeSetBg(t, k, k, null); } catch (_) { } }
          }
...
function generateFullProposal(formData) {
  try {
    ...
    var sys = 'Tu es un consultant Icam. Retourne STRICTEMENT un JSON {contexte, demarche, phases, phrase}.';
    var user = 'A partir du brief ci-dessous, renvoie un JSON {contexte, demarche, phases, phrase}. Brief:\n\n' + brief;

    var chosenModel = resolveDeepseekModel_(formData && formData.deepseekModel);
    var llm = callLLM_(chosenModel, user, sys, 0.3, { model: chosenModel, maxTokens: 1200 });
    if (!llm.success) return llm;

    var sections;
    try { sections = JSON.parse(llm.content); }
    catch (e) { return { success: false, error: 'La reponse IA nest pas un JSON valide: ' + e.message }; }
    ...
    var u = applyUpdatesToDoc_(copy.documentId, updates, { removeAllHighlight: true });
...
function tokensApprox(chars) { return Math.ceil((chars || 0) / 4); }
function countDocChars_(docId) {
  if (!docId) return 0;
  try { return DocumentApp.openById(docId).getBody().getText().length || 0; }
  catch (_) { return 0; }
}
function estimateApiCost(formData, options) {
  ...
  var extraIds = (options.extraInputDocIds && options.extraInputDocIds.slice) ? options.extraInputDocIds : [];

  var brief = [
    'Entreprise: ' + ((formData && formData.entrepriseNom) ? formData.entrepriseNom : ''),
    'Probleme: ' + ((formData && formData.ia_probleme) ? formData.ia_probleme : ''),
    'Solution: ' + ((formData && formData.ia_solution) ? formData.ia_solution : ''),
    'Objectifs: ' + ((formData && formData.ia_objectifs) ? formData.ia_objectifs : ''),
    'Thematique: ' + ((formData && formData.thematique) ? formData.thematique : ''),
    'Duree: ' + ((formData && formData.dureeProjet) ? formData.dureeProjet : ''),
    'Adresse: ' + ((formData && formData.entrepriseAdresse) ? formData.entrepriseAdresse : '')
  ].join('\n');

  var systemPrompt = 'Tu es un consultant Icam. Retourne STRICTEMENT un JSON {contexte, demarche, phases, phrase}.';
  var userPrompt = 'A partir du brief ci-dessous, renvoie un JSON {contexte, demarche, phases, phrase}. Brief:\n\n' + brief;

  var extraChars = 0;
  for (var i = 0; i < extraIds.length; i++) extraChars += countDocChars_(extraIds[i]);
  ...
  return {
    ...
    input: { chars: inputChars, tokens: inputTokens, price_per_M: priceIn, cost: +costIn.toFixed(6) },
    output: { pages: pagesTarget, tokens: outputTokens, price_per_M: pricing.out, cost: +costOut.toFixed(6) },
    total: total
  };
}
...
function getOrCreateCostSheet_() {
  ...
  if (sheetId) {
    try { ss = SpreadsheetApp.openById(sheetId); } catch (_) { ss = null; }
  }
  if (!ss) {
  ...
  if (sh.getLastRow() === 0) {
    ...
    var current = sh.getRange(1, 1, 1, Math.max(sh.getLastColumn(), headerWidth)).getValues()[0];
    var mismatch = false;
    for (var i = 0; i < headerWidth; i++) {
      if ((current[i] || '') !== COST_LOG_HEADER[i]) { mismatch = true; break; }
    }
    if (mismatch) {
...
function logCostEntry_(entry, formData) {
  try {
    ...
    var sh = got.sheet || got.ss.getSheetByName('log') || got.ss.getActiveSheet();
    sh.appendRow([
      new Date(),
      entry.entryType || '',
      entry.model || '',
      entry.cacheHit === true ? 'hit' : entry.cacheHit === false ? 'miss' : (entry.cacheHit || ''),
